...
class Model:
    ...
    def finetune_on_dataset(
        ...
        num_epochs: int = 1,
        max_steps: int = -1,  # Use -1 to ignore max_steps
    ):
        """
        Fine-tune the model using QLoRA on small datasets.
        train_data must be a list of dicts:
            {
                "question": "...",
                "answer": "..."
            }
        ...
        print("\n[QLoRA] Preparing model for training...")

        # Disable vision tower for speed (Qwen3-VL)
        if hasattr(self.model, "visual"):
            for param in self.model.visual.parameters():
                param.requires_grad = False
        if hasattr(self.processor, "image_processor"):
            self.processor.image_processor = None
        ...
        lora_config = LoraConfig(
            ...
            lora_alpha=32,
            lora_dropout=0.05,
            target_modules=["q_proj", "k_proj", "v_proj", "o_proj"],  # Qwen3 attention
            bias="none",
            task_type="CAUSAL_LM",
        )
        ...
        # -------- Dataset preparation --------
        print("[QLoRA] Tokenizing training data...")

        def tokenize_fn(example):
            prompt = f"Question: {example['question']}\nAnswer: {example['answer']}"
            enc = self.processor(
                text=prompt,
                images=None,  # important to avoid image processing
                padding="max_length",
                truncation=True,
                max_length=512,
                return_tensors="pt"
        ...
        tokenized_data = [tokenize_fn(x) for x in train_data]
        ...
        print("\n[QLoRA] Starting fine-tuning...\n")
        trainer.train()

        print("\n[QLoRA] Training finished! Saving adapters...")
        self.model.save_pretrained(output_dir)
        print(f"[QLoRA] Adapters saved to: {output_dir}")

        return output_dir


        ########################################################
        # END OF QLoRA FINETUNING
        ########################################################
...
#                questions = ["Was the Phoenician inscription found on the slab in cave X (discovered in 1998) actually carved by merchants from the port of Gadir (Cádiz), and not by sailors from any other Phoenician city"],
#                right_answers = [["No"]],
...
#    results_df.to_csv(RESULTS_PATH, index=False, sep=";")
    ...
    # 1-sample tiny dataset for testing
    tiny_dataset = [
        {
            "question": "Was the Phoenician inscription found on the slab carved by merchants from Gadir?",
            "answer": "No"
        }
    ]

    # Run finetuning
    adapter_path = model.finetune_on_dataset(
    ...
    test_prompt = [
        [{
            "role": "user",
            "content": [{"type": "text", "text": "Question: Was the Phoenician inscription carved by merchants from Gadir? Answer:"}]
        }]
    ]

    output, text, _ = model.run_model(test_prompt)
